...
def pagina_inicial():
    ...
    if request.method == 'POST':
        ...
        if tipo_download == 'video':
            nome_arquivo = mp3_downloader.delay(url, DOWNLOAD_FOLDER)
        else:
            nome_pasta = playlist_downloader.delay(url, DOWNLOAD_FOLDER)

        flash('seu donwload foi iniciado e logo sera salvo no servidor')
